...
lore.ui.setUpMetadataMenu = function(the_grid, gridname){
    ...
    if (gridname == "lore.ui.nodegrid") {
        for (var i = 0; i < lore.ore.resource_metadata_props.length; i++) {
            lore.ui._make_menu_entry(
                addMetadataMenu, 
                gridname, 
                lore.ore.resource_metadata_props[i], 
                "add"
            );
            lore.ui._make_menu_entry(
                remMetadataMenu, 
...
lore.ui.initGraphicalView = function () {
    ...
    lore.ore.graph.gSelectionListener = 
        new lore.ore.graph.SelectionProperties(lore.ore.graph.Graph);
    ...
    // selectedFigure is last selected figure - updated by SelectionProperties.js
...
lore.ui.initExtComponents = function (){
    ...
    window.addEventListener("DOMFrameContentLoaded",lore.anno.handleFrameLoad,true);
	
    // set up welcome tab contents
    lore.ui.welcometab.body.update("<iframe height='100%' width='100%' "
      + "src='chrome://oaiorebuilder/content/welcome.html'></iframe>");
}
...
lore.ui.initTimeline = function (){
    ...
    if (typeof Timeline !== "undefined") {
        ...
        lore.anno.annotimeline = 
            Timeline.create(document.getElementById("annotimeline"), bandConfig);
