/* 
 * Listens for commands such as delete, undo, redo and updates the workflowLookup object that is used
...
oaiorebuilder.CommandListener.prototype.stackChanged=function(event){
	// remove the url from workflowLookup if node is deleted, add it back if it is undone
	...
	if(0!=(details&(draw2d.CommandStack.POST_EXECUTE)))
	{
		if(comm instanceof draw2d.CommandDelete){
			delete workflowLookup[comm_fig.url];
		}
	}
	else{if(0!=(details&(draw2d.CommandStack.POST_UNDO)))
	{
		if(comm instanceof draw2d.CommandDelete){
			workflowLookup[comm_fig.url] = comm_fig.getId();
		}
	}
	else{if(0!=(details&(draw2d.CommandStack.POST_REDO)))
	{
		if(comm instanceof draw2d.CommandDelete){
			delete workflowLookup[comm_gif.url];
		}
	}}}
};