...
$.imgAreaSelect = function (img, ctx, options) {
    ...
    function div() {
        ...
        lore.util.ignoreElementForXP(div);
    ...
    function adjust() {
        ...
        if ($().jquery == '1.3.2' && $.browser.safari && position == 'fixed') {
            ...
            imgOfs.left += max(document.documentElement.scrollLeft, $('body', context).scrollLeft());
        }
        ...
        parOfs = $.inArray($parent.css('position'), ['absolute', 'relative']) + 1 ?
            ...
            position == 'fixed' ?
                { left: $(context).scrollLeft(), top: $(context).scrollTop() } :
                { left: 0, top: 0 };
    ...
    function update(resetKeyPress) {
        ...
        $($outer[0], context).css({ left: left, top: top,
            width: selection.x1, height: imgHeight });
        $($outer[1], context).css({ left: left + selection.x1, top: top,
            width: w, height: selection.y1 });
        $($outer[2], context).css({ left: left + selection.x2, top: top,
            width: imgWidth - selection.x2, height: imgHeight });
        $($outer[3], context).css({ left: left + selection.x1, top: top + selection.y2,
        ...
        case 8:
            $($handles[4],context).css({ left: w / 2 });
            $($handles[5],context).css({ left: w, top: h / 2 });
            $($handles[6],context).css({ left: w / 2, top: h });
            $($handles[7],context).css({ top: h / 2 });
        case 4:
        ...
        if (resetKeyPress !== false) {
            if ($.imgAreaSelect.keyPress != docKeyPress)
                $(context).unbind($.imgAreaSelect.keyPress,
                    $.imgAreaSelect.onKeyPress);

            if (options.keys)
                $(context)[$.imgAreaSelect.keyPress](
    ...
    function docMouseUp(event) {
        ...
        $(context).unbind('mousemove', selectingMouseMove);
    ...
    function areaMouseDown(event) {
        ...
        if (resize) {
            $('body',context).css('cursor', resize + '-resize');

            x1 = viewX(selection[/w/.test(resize) ? 'x2' : 'x1']);
            y1 = viewY(selection[/n/.test(resize) ? 'y2' : 'y1']);

            $(context).mousemove(selectingMouseMove)
            ...
            $(context).mousemove(movingMouseMove)
                .one('mouseup', function () {
                    options.onSelectEnd(img, getSelection());

                    $(context).unbind('mousemove', movingMouseMove);
                    $box.mousemove(areaMouseMove);
                });
        }
        else
    ...
    function startSelection() {
        ...
        $(context).unbind('mouseup', cancelSelection)
    ...
    function cancelSelection() {
        $(context).unbind('mousemove', startSelection);
        hide($box.add($outer));
    ...
    function imgMouseDown(event) {
        ...
        $(context).one('mousemove', startSelection)
    ...
    function setOptions(newOptions) {
        ...
        for (i = 0; i++ < 4;)
            $($border[i-1],context).addClass(options.classPrefix + '-border' + i);
        ...
        if (o = options.borderColor1)
            $($border[0],context).css({ borderStyle: 'solid', borderColor: o });
        if (o = options.borderColor2)
            $($border[1],context).css({ borderStyle: 'dashed', borderColor: o });
    ...
    if ($.browser.opera)
        $areaOpera = div(this.context).css({ width: '100%', height: '100%',
